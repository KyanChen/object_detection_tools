...
def cut_with_label(file_list):
    for id, img_file in tqdm.tqdm(enumerate(file_list)):
        ...
        csv_file = img_file.replace(IMG_FORMAT, 'csv')
        df = pd.read_csv(csv_file)
        bboxes = df['box']
        bboxes = np.array([eval(x) for x in bboxes])
        if len(bboxes) != 0:
            if bboxes[0][2] < 1:
                print(csv_file)
        ...
        if img_h < SLICE_SIZE[1]*1.1 or img_w < SLICE_SIZE[0]*1.1:
            continue
        w_list = []
        h_list = []
        w_coor = 0
        ...
        for w_start in w_list:
            for h_start in h_list:
                ...
                centers = (bboxes[:, 0:2] + bboxes[:, 2:]) / 2.0
                ...
                if current_boxes is None:
                    ...
                    np.savetxt(img_save_path.replace(IMG_FORMAT, 'txt'), current_boxes, fmt='%.4f %.4f %.4f %.4f')


if __name__ == '__main__':
    PATCH_PATH = r'H:\optimal\Airbus Ship Detection Challenge\images'
    OVERLAP_RATE = 0.1
    SLICE_SIZE = (512, 512)
    SLICE_SAVED_PATH = r'H:\optimal\Airbus Ship Detection Challenge\20211110_Slice'  # 切片保存的路径
    IMG_FORMAT = 'jpg'
